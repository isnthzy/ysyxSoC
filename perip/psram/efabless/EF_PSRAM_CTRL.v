...
module PSRAM_READER_QPI (
    ...
    // Sample with the negedge of sck
    wire[1:0] byte_index = {counter[7:1] - 8'd7}[1:0]; //例如counter (14/2)-7=0开始索引
    always @ (posedge clk)
...
module QPI_MODE (
    ...
    input   wire            open_qpi,
    output  wire            done,
    output  reg             qpi_mode, //qpi_mode拉高为打开
    ...
    always @*
        case (state)
            IDLE: if(open_qpi) nstate =   QPI; else nstate = IDLE;
